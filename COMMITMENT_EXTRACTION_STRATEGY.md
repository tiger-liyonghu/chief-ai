...
## 2026-04-01 | v1.1（更新 baseline + 新增决策 + Tier 2 触发条件）
...
| 指标 | 初始 (03-30) | 当前 (04-01) | 世界级标准 | 差距 |
|------|-------------|-------------|-----------|------|
| Precision | 58% | **94.5%** | >95% | -0.5 |
| Recall | 63% | **65.9%** | >85% | -19 |
| F1 | 60% | **77.7%** | >90% | -12 |
| Pre-filter | 97% | 97% | >95% | OK |

> Precision 已接近世界级。**当前瓶颈是 Recall (65.9%)**——每 3 个真实承诺漏掉 1 个。
> Tier 2 (Reasoner) 的核心任务就是补上这 ~20pp 的 recall 差距。
...
| 决策 | 选择 | 理由 | 日期 |
|------|------|------|------|
| Precision vs Recall 优先 | Precision | 误报伤害信任 5-10x | 03-30 |
| 单模型 vs 双模型 | 双模型（短期单、中期双） | 单模型 precision 天花板 ~85% | 03-30 |
| 中英文分 pipeline vs 合一 | 合一 | 现代 LLM 原生多语言 | 03-30 |
| Fine-tune vs Prompt | 先 prompt（数据不够 fine-tune） | 需 500+ 反馈数据才值得 fine-tune | 03-30 |
| 实时 vs 异步 | 冷启动实时，日常异步 | Wow moment 要快，日常可以后台 | 03-30 |
| **统一 prompt 三合一 → 否决** | 各 prompt 保持独立 | 实验证明三合一导致输出语言不稳定（title 中英文乱跳）、注意力稀释；Shortwave 验证了"多个专注的并行调用 > 一个大而全的调用"的模式 | 04-01 |
| **Prompt 语言 → 匹配邮件语言** | Few-shot 中英文各一组 | 跨 35 语言研究：prompt 语言匹配内容语言时准确率提升最高 50%，处理时间快 25-35% | 04-01 |
| **本体模型 vs Prompt → 解耦** | 本体数据层先上，prompt 不动 | 本体（Signal/Topic/Commitment 表结构）和 prompt（提取逻辑）是独立的事，不需要绑定上线 | 04-01 |

---

## 八-B、Tier 2 触发条件（2026-04-01 新增）

Tier 1 (DeepSeek Chat) 处理所有邮件，Tier 2 (DeepSeek Reasoner) 只在以下条件触发：

| 触发条件 | 判定规则 | 原因 |
|---------|---------|------|
| **零提取异常** | 邮件 >200 字 + 未被预过滤 + Tier 1 提取 0 承诺 | 长邮件大概率有内容，提取为零说明 Tier 1 没读懂 |
| **结构复杂** | 包含关键词 (minutes/action items/纪要/跟进事项/follow-up) 或收件人 >3 人 | 会议纪要和多方讨论是 Tier 1 漏检最高的类别 (R=20-25%) |
| **低置信度聚集** | Tier 1 有提取但所有置信度 <0.75 | 模型自己不确定，交给推理模型重新判断 |
| **高利害关系** | 发件人是 VIP 或包含关键词 (contract/deadline/legal/合同/逾期) | 这类邮件漏一个代价很大 |

**流量预估**（日均 50 封邮件的典型用户）：
```
50 封 → 预过滤扔掉 30 封 → Tier 1 处理 20 封 → Tier 2 触发 3-5 封 (~15%)
```

**与本体的关系**：
```
信号处理框架（怎么提取）──写入──→ 本体模型（怎么存）
预过滤 → Tier 1 → Tier 2           Signal → Topic → Commitment
       处理流程                          数据结构
```
两层提取结果都写入同一套本体。Tier 2 可补充或覆盖 Tier 1 的结果。
