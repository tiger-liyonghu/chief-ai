import { ToolDefinition, ToolContext, getLLMClient } from './types'
...
export async function execute(ctx: ToolContext, name: string, args: any): Promise<string | null> {
  switch (name) {
    ...
    case 'get_cultural_brief': {
      const { generateCulturalBrief } = await import('../travel-brain')
    ...
    case 'get_local_recommendations': {
      const { generateLocalRecommendations } = await import('../travel-brain')
