...
import { Globe, Clock, Shield, Trash2, Download, Send, Check, Plus, X, Mail, User, MessageCircle, Loader2, Bot, Eye, EyeOff, Zap } from 'lucide-react'
...
function SettingsContent() {
  ...
  const sendDigestNow = async () => {
    ...
    try {
      ...
      // Silently fail — digest API may not exist yet
    } finally {
    ...
    <div>
      ...
      <div className="p-8 max-w-2xl">
        ...
        <div className="space-y-8">
          ...
          {/* Digest Section */}
          <section>
            <h2 className="text-sm font-semibold text-text-primary uppercase tracking-wider mb-4">Daily Digest</h2>
            <div className="bg-white rounded-2xl border border-border divide-y divide-border">
              <div className="p-5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Clock className="w-5 h-5 text-text-tertiary" />
                  <div>
                    ...
                    <p className="text-xs text-text-tertiary">
                      {digestEnabled
                        ? `Sends daily at ${settings.daily_brief_time}`
                        : 'Digest is paused'}
                    </p>
                  </div>
                </div>
              ...
              <div className="p-5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-tertiary">Get an immediate summary of your inbox and tasks</p>
                  </div>
                </div>
                <button
                  onClick={sendDigestNow}
                  disabled={digestSending}
                  className="text-sm font-medium text-primary bg-primary/10 px-4 py-1.5 rounded-lg hover:bg-primary/20 transition-colors disabled:opacity-50"
                >
